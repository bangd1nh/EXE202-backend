...
export const getAllPhotographers = async () => {
  const photographers = await PhotographerProfile.find()
    .populate("PhotographerId", "Email FirstName LastName PhoneNumber Avatar Username")
...
export const getServiceByPhotographersId = async (photographerId) => {
  const service = await PhotographerProfile.findById(photographerId).select("Services -_id").populate("Services");
  if (!service) {
...
export const updatePhotographerProfile = async (userId, expY, location, device, description) => {
...
export const uploadImageForPhotographer = async (file, userId) => {
  return new Promise((resolve, reject) => {
    let stream = cloudinary.uploader.upload_stream({ folder: "photographer_images" }, async (error, uploadResult) => {
      if (error) {
      ...
      try {
        ...
        if (!photographer) {
          return reject(dataResponse(404, "Photographer not found", null));
        }

        let photograph;
        if (photographer.PhotoGraphs) {
          photograph = await Photographs.findById(photographer.PhotoGraphs);
        }
...
export const createServiceForPhotographer = async (photographerId, data) => {
  ...
  const photographer = await PhotographerProfile.findOne({ PhotographerId: photographerId });
  if (!photographer) {
