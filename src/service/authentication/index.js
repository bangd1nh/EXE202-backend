...
export const sendToken = async (email, token) => {
  try {
    const sent = await sendEmail(email, "Please verify your email", `${process.env.BASE_VERCEL_URL}/verify/${token}`);
...
export const login = async (usernameOrEmail, password) => {
  try {
    ...
    if (user.verify === false) {
      return dataResponse("you must verify your email to able to login", 400, null);
    }
